function displayCardsDynamically(collection) {
    ...
    db.collection(collection)
        .where("stress-score", "<=", 9)                     // Query of database to check appropriate stress score
        .where("stress-score", ">", 4)                      // Query of database to check appropriate stress score
        .limit(3)                                           // Read from firestore to receive user stress score
        .get()                                              
        .then(allActivities => {
